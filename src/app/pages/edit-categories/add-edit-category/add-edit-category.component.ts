...
export class AddEditCategoryComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    this.subscription = this.route.params.subscribe(params => {
      ...
      if (this.category_id && this.type === 'add') {
        this.router.navigate(['/case', this.case_id, 'categories', this.type]);
      }
    });
  ...
  private setFormData(): void {
    ...
    this.other_penalties.setValue(0);
    if (current_tax) {
  ...
  addUpdateCategory(): void {
    const payload = {
      ...
      disputed_t1_ta_id: this.tax,
      taxable_income: this.income,
      income_subject_to_gnp: this.gnp,
      other_amounts_payable: this.other_amounts,
      credits_applied_on_filing: this.credits,
      federal_non_refundable_tax_credits: this.federal,
      provincial_non_refundable_tax_credits: this.provincial,
      other_penalties: this.other_penalties
    };
    if (this.category_id) {
